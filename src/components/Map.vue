<script setup>
    ...
    onMounted(() => {
        ...
        storedCoordinates.forEach(({ lat, lon, title, description, index }) => {
            addMarker(lat, lon, title, description, index);
        });
    });
    ...
    const save_form = async () => {
        ...
        const index = form.value.index; // Get the index from the form
        ...
        try {
            ...
            if (response.data.results && response.data.results.length > 0) {
                ...
                addMarker(lat, lon, address, description, index);
                storeCoordinates(lat, lon, address, description, index);
            } else {
    ...
    const addMarker = (lat, lon, title, description, index) => {
        if (map.value) {
            ...
            marker.getElement().addEventListener('click', () => {
                selectedMarker.value = { marker, title, description, lat, lon, index };
    ...
    const storeCoordinates = (lat, lon, title, description, index) => {
        const coordinates = getStoredCoordinates() || [];
        coordinates.push({ lat, lon, title, description, index });
    ...
    const deleteMarker = () => {
        if (selectedMarker.value) {
            const { marker, lat, lon, title, description, index } = selectedMarker.value;
    
            marker.remove();
    
            let coordinates = getStoredCoordinates();
            coordinates = coordinates.filter(coord => coord.lat !== lat || coord.lon !== lon || coord.title !== title || coord.description !== description || coord.index !== index);
            localStorage.setItem('coordinates', JSON.stringify(coordinates));
    ...
    const updateDescription = () => {
        if (selectedMarker.value) {
            const { lat, lon, title, description, index } = selectedMarker.value;
            let coordinates = getStoredCoordinates();
            coordinates = coordinates.map(coord => {
                if (coord.lat === lat && coord.lon === lon && coord.title === title) {
                    return { ...coord, description, index: activeIndex.value };
                }
                return coord;
            });
